...
CREATE TABLE CUSTOMER (
    ...
    CONSTRAINT pk_cutomer PRIMARY KEY (CUSTNO)
);
...
CREATE TABLE LOCATION (
    ...
    CONSTRAINT fk_location_facno_facility_facno FOREIGN KEY (FACNO)
...
CREATE TABLE RESOURCETBL (
    ...
    RATE DEC(13 , 4 ) NOT NULL,
    CONSTRAINT pk_respurceTable PRIMARY KEY (RESNO),
...
CREATE TABLE EVENTREQUEST (
    ...
    ESTCOST DEC(13 , 4 ) NOT NULL,
    ESTAUDIENCE INTEGER NOT NULL,
    BUDNO VARCHAR(8),
    CONSTRAINT pk_eventRequest PRIMARY KEY (EVENTNO),
    CONSTRAINT fk_eventRequest_facno_facility_facno FOREIGN KEY (FACNO)
        REFERENCES facility (FACNO)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_eventRequest_custno_customer_custno FOREIGN KEY (CUSTNO)
        REFERENCES customer (CUSTNO)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT chk_authstatus CHECK (STATUS IN ('Pending' , 'Denied', 'Approved')),
...
CREATE TABLE EVENTPLAN (
    ...
    CONSTRAINT fk_eventPlan_eventno_eventRequest_eventno FOREIGN KEY (EVENTNO)
        REFERENCES eventRequest (EVENTNO)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_eventPlan_empno_employee_empno FOREIGN KEY (EMPNO)
...
CREATE TABLE EVENTPLANLINE (
    ...
    CONSTRAINT fk_eventPlanLine_planno_eventPlan_planno FOREIGN KEY (PLANNO)
        REFERENCES EVENTPLAN (PLANNO)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_eventPlanLine_locno_location_locno FOREIGN KEY (LOCNO)
        REFERENCES LOCATION (LOCNO)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_eventPlanLine_resno_resourceTable_resno FOREIGN KEY (RESNO)
        REFERENCES RESOURCETBL (RESNO)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT chk_timestart CHECK (TIMESTART < TIMEEND)
);
